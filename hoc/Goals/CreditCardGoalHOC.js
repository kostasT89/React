...
function CreditCardGoalHOC(WrappedComponent) {
  class WrappedCreditCardGoalComponent extends Component {
    ...
    componentDidMount() {
      ...
      dispatch(getYodleeAccount(accountId || selectedGoal.accountId, yodleeContainers.creditCard));
    ...
    _updateFormValues = (connectedFormValues, change) => {
      ...
      } = WrappedCreditCardGoalComponent;
      // Initialize:
    ...
    _onFormSubmit(goal) {
      ...
      if (accountId) {
        dispatch(submitGoalForm(formattedGoal, accountId, goalTypes.creditCard));
        return;
      }
      if (!isUndefined(goalId)) {
    ...
    render() {
      ...
      // Calc Initial Values for Editing Credit-Card Goal
      let selectedGoal;
      if (!isUndefined(goalId)) {
        selectedGoal = goals.find(goal => goal.id.toString() === goalId.toString());
      }
      ...
      // For initialValues of Credit-Card Goal Editing
  ...
  function mapStateToProps(state) {
    return {
      data: state.creditCardGoal,
      isSubmitting: state.goals.isSubmitting,
      goals: state.dashboard.goals
    };
  }

  return connect(mapStateToProps)(WrappedCreditCardGoalComponent);
}

export default CreditCardGoalHOC;
