...
class CreditCardForm extends Component {
  ...
  render() {
    const {
      ...
      pristine,
      invalid,
      submitting, // from redux from
      repaymentMethod,
      onRepaymentMethodClick,
      change,
      connectedValues,
      onFieldChange,
      isSubmitting, // created by us on submit
      actionStatus
    } = this.props;
    ...
    return (
      <form className="lc-credit-card-goal-form" onSubmit={handleSubmit}>
        {/* <!--FIELDS--> */}
        <div className="row">
          {/* <!--Nickname--> */}
          <div className="lc-column lc-column--left columns small-10 small-offset-1">
            <Field name={attrs.nickname}
                   ...
                   subtext={cms['goals.ccGoal.field.nickname.subtext']}
                   subtext2={cms['goals.field.nickname.subtext2']}
                   validate={[required]} />
          </div>
          {/* <!--HEADING--> */}
          <div className="lc-column lc-column--left columns small-10 small-offset-1">
            <h3 className="lc-credit-card-goal-form__h3">{cms['goals.currentPaymentStructure']}</h3>
          </div>
          {/* <!--Current Balance--> */}
          <div className="lc-column lc-column--right columns small-10 small-offset-1">
            <Field name={attrs.balanceAtGoalStart}
              ...
              label={cms['goals.field.balanceAtGoalStartOfAccount.label']}
          ...
          <div className="lc-column lc-column--left columns small-10 small-offset-1">
            <Field name={attrs.interestRate}
              ...
              label={cms['goals.field.interestRateOnAccount.label']}
              onChangeCB={node => onFieldChange(node, change, connectedValues)}
              validate={[required]} />
          </div>
          {/* <!--DIVIDER--> */}
          ...
          <div className="lc-column lc-column--left columns small-10 small-offset-1">
            <Field name={attrs.remainingTermInMonths}
              ...
              label={cms['goals.field.repaymentTerm.label']}
              onChangeCB={node => onFieldChange(node, change, connectedValues)}
              validate={[required]} />
          </div>
          ...
          <div className="lc-column lc-column--left columns small-10 small-offset-1">
            <Field name={attrs.remainingTotalInterest}
                   ...
                   type="text"
                   mask={fieldMasks.currency}
                   label={cms['goals.field.totalInterestToBePaid.label']}
                   validate={[required, dollarAmount]}
                   isDisabled />
          </div>
          ...
          <RepaymentMethodFields repaymentMethod={repaymentMethod}
                                 ...
                                 header={cms['goals.field.ccRepaymentSelection.label']}
                                 {...errors} />
          {/* <!--BUTTONS--> */}
          <div className="lc-column lc-column--buttons columns small-10 small-offset-1">
            {actionStatus !== ActionStatus.edit ?
              <Link to={populateGoalSelectionRoute(goalTypes.creditCard)} >
...
const DecoratedForm = reduxForm({
  form: reduxForms.creditCardGoalForm
})(CreditCardForm);


const selector = formValueSelector(reduxForms.creditCardGoalForm);
